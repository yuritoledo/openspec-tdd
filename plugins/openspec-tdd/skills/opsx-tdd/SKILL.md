...
4. **Write REAL failing tests** (solid RED, not forced fail)
   ...
   - Existing sibling code and the project's **canonical test reference** (the file the repo's
     CLAUDE.md/AGENTS.md points to as the pattern, e.g. a representative `*.test.tsx`).

   For each scenario group:
   - If the target file does not exist, create it using the project's test conventions: the
     repo's custom `render()`/wrapper helper, `screen`, `userEvent`, and the shared mock setup —
     never a bare testing-library render for component tests.
...
5. **Run tests and confirm RED**
   ...
   Run the project's test runner against each written file (`bun test --run <file>`,
   `npx vitest run <file>`, or `npm test -- <file>` — match the repo). Where the runner
...
- Component tests use the project's custom `render()`/wrapper and query by `data-testid`, not a
  bare testing-library render.
